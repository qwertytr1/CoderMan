...
namespace СoderMan
{
    public partial class lvl4 : Form
    {
        ...
        private bool IsValidMove(int deltaX, int deltaY)
        {
            ...
            {
                // Check for collision with any obstacle PictureBox
                foreach (Control obstacle in Controls)
                {
                    if (obstacle is PictureBox && obstacle != pictureBox2)
                    {
                        if (CollidesWithPictureBox(newX, newY, (PictureBox)obstacle))
                        {
                            // Collision detected, handle it here
                            textBox1.AppendText($"Collision with object at X:{((PictureBox)obstacle).Location.X}, Y:{((PictureBox)obstacle).Location.Y}{Environment.NewLine}");
        ...
        private bool CollidesWithPictureBox(int x, int y, PictureBox pictureBox)
        {
            bool collides = (x < pictureBox.Right &&
                               x + pictureBox2.Width > pictureBox.Left &&
                               y < pictureBox.Bottom &&
                               y + pictureBox2.Height > pictureBox.Top);

            Console.WriteLine($"Collision Result: {collides}");
            Console.WriteLine();
